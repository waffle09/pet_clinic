...
// echo $contact;
$adress = filter_var(trim($_POST['kind']));
...
$conn->query("UPDATE `doctor` SET `service_id` = $adress, `doc_name` = $cl_name, `contact` = $id WHERE `doctor`.`certificate_number` = $num_red_pet");
...
header('location: http://localhost:81/red_doc.php');
?>
